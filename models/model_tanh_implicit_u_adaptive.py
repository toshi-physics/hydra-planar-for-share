...
from src.system_adaptive import System
...
def main():
    ...
    xi        = parameters["B"]        # elastic coupling constant
    ...
    system = System(grid_size, fourier_operators) #this system initiated with atol=1e-6, rtol=1e-3, mindt=1e-6, maxdt=5e-2
    ...
    # Define disps
    system.create_term("ux", [("ux", None)], [1, 0, 0, 0, 0])
    system.create_term("uy", [("uy", None)], [1, 0, 0, 0, 0])
    # Define epsilon_iso
    system.create_term("epsilon_iso", [("ux", None)], [1, 0, 1, 0, 0])
    system.create_term("epsilon_iso", [("uy", None)], [1, 0, 0, 1, 0])

    system.create_term("epsilon_e", [("epsilon_iso", None)], [0.5/epsilont, 0, 0, 0, 0])
    system.create_term("epsilon_e", [("epsilon_iso", (np.abs, None))], [0.5/epsilont, 0, 0, 0, 0])
    ...
    system.create_term("c", [("epsilon_e", (np.tanh, None))], [beta, 0, 0, 0, 0])
    #system.create_term("c", [("epsilon_e", None)], [beta, 0, 0, 0, 0])
    ...
    #S = np.random.rand(mx, my)
    #theta = np.pi * np.random.rand(mx, my)
    #Qxx.set_real(S*np.cos(2*theta)/2)
    #Qxx.synchronize_momentum()
    #Qxy.set_real(S*np.sin(2*theta)/2)
    ...
    dt_next=dt #for the first time step choose the given dt
    t=0
    output_times = np.arange(dt_dump, T+dt_dump, dt_dump)
    i=0
    np.savetxt(savedir+'/data/'+'c.csv.'+ str(np.int32(t/dt_dump)), c.get_real(), delimiter=',')
    np.savetxt(savedir+'/data/'+'Qxx.csv.'+ str(np.int32(t/dt_dump)), Qxx.get_real(), delimiter=',')
    np.savetxt(savedir+'/data/'+'Qxy.csv.'+ str(np.int32(t/dt_dump)), Qxy.get_real(), delimiter=',')
    np.savetxt(savedir+'/data/'+'eiso.csv.'+ str(np.int32(t/dt_dump)), eiso.get_real(), delimiter=',')
    np.savetxt(savedir+'/data/'+'uy.csv.'+ str(np.int32(t/dt_dump)), uy.get_real(), delimiter=',')
    np.savetxt(savedir+'/data/'+'ux.csv.'+ str(np.int32(t/dt_dump)), ux.get_real(), delimiter=',')
    
    with tqdm(total=T, unit="time") as pbar:
        while t<T:
            savedata=False

            if t < output_times[i] and t+dt_next>output_times[i]:
                dt_next = output_times[i]-t
                i+=1
                savedata=True
            ...
            update_strains(ux, uy, Qxx, Qxy, alpha, inv_umatrix, k_grids)
            
            dt_next = system.update_system(dt_next)
...
def set_aster(fieldxx, fieldxy, size, dr, nem_length):
    center1 = dr*size/2
    center2 = center1 - np.array([0, nem_length*15/dr[0]])
    center3 = center1 + np.array([0, nem_length*15/dr[0]])
...
def update_strains(fux, fuy, fQxx, fQxy, alpha, inv_umatrix, k_grids):
    iqx = 1j*k_grids[0]
    iqy = 1j*k_grids[1]
    ...
    # shape 2, nx, ny
    b_vector = np.array([alpha*qQxx*iqx + alpha*iqy*qQxy, -alpha*qQxx*iqy + alpha*iqx*qQxy]) 
    b_vector = np.transpose(b_vector, (1,2,0)) #shape nx, ny, 2_col
    
    u_vector = np.matvec(inv_umatrix, b_vector)
    
    fux.set_momentum(u_vector[:,:,0])
    fux.synchronize_real()
    fuy.set_momentum(u_vector[:,:,1])
    fuy.synchronize_real()
