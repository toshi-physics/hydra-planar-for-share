...
def set_aster(fieldxx, fieldxy, size, dr, nem_length):
    center1 = dr*size/2
    center2 = center1 - np.array([0, nem_length*6/dr[0]])
    center3 = center1 + np.array([0, nem_length*6/dr[0]])
...
def invert_disp_matrix(k_grids, mu, lambd):
    qx = k_grids[0]
    qy = k_grids[1]
    zero_modes = np.where(qx==0, True, False)*np.where(qy==0, True, False)
    eps = 0.1 #small value replacing zero. We will set these modes to zero later manually. This is just so we can invert the matrix.
    qx[zero_modes] = eps
    qy[zero_modes] = eps
    u_matrix = np.array([[(lambd+2*mu)*qx**2 + mu*qy**2, (mu+lambd)*qx*qy], [(mu+lambd)*qx*qy, (2*mu+lambd)*qy**2 + mu*qx**2]]) #this has shape [2_row, 2_col, nx, ny]
