...
def main():
    ...
    system.create_field('sigma_xy', k_list, k_grids, dynamic=False)
    system.create_field('sigma_xx', k_list, k_grids, dynamic=False)
    system.create_field('sigma_yy', k_list, k_grids, dynamic=False)

    system.create_field('ux', k_list, k_grids, dynamic=False)
    system.create_field('uy', k_list, k_grids, dynamic=False)
    ...
    system.create_field('epsilon_ext', k_list, k_grids, dynamic=False)
    system.create_field('sate', k_list, k_grids, dynamic=False)
    ...
    system.create_term("sigma_xx", [("epsilon_xx", None)], [(2.0*mu)+lambd, 0, 0, 0])
    system.create_term("sigma_xx", [("epsilon_yy", None)], [lambd, 0, 0, 0])
    system.create_term("sigma_xx", [("Qxx", None)], [alpha, 0, 0, 0])
    # Define xy stress
    system.create_term("sigma_xy", [("epsilon_xy", None)], [(2.0*mu), 0, 0, 0])
    system.create_term("sigma_xy", [("Qxy", None)], [alpha, 0, 0, 0])
    # Define yy stress
    system.create_term("sigma_yy", [("epsilon_yy", None)], [(2.0*mu)+lambd, 0, 0, 0])
    system.create_term("sigma_yy", [("epsilon_xx", None)], [lambd, 0, 0, 0])
    system.create_term("sigma_yy", [("Qxx", None)], [-alpha, 0, 0, 0])
    # Define x displacements
    system.create_term("ux", [("sigma_xx", None)], [1/B, 0, 1, 0])
    system.create_term("ux", [("sigma_xy", None)], [1/B, 0, 0, 1])
    # Define y displacements
    system.create_term("uy", [("sigma_yy", None)], [1/B, 0, 0, 1])
    system.create_term("uy", [("sigma_xy", None)], [1/B, 0, 1, 0])
    # Define strains
    system.create_term("epsilon_xx", [("ux", None)], [1, 0, 1, 0])
    system.create_term("epsilon_yy", [("uy", None)], [1, 0, 0, 1])
    system.create_term("epsilon_xy", [("ux", None)], [0.5, 0, 0, 1])
    system.create_term("epsilon_xy", [("uy", None)], [0.5, 0, 1, 0])
    system.create_term("epsilon_ext", [("epsilon_xx", None)], [1, 0, 0, 0])
    system.create_term("epsilon_ext", [("epsilon_yy", None)], [1, 0, 0, 0])
    system.create_term("epsilon_ext", [("epsilon_xx", (np.abs, None))], [1, 0, 0, 0])
    system.create_term("epsilon_ext", [("epsilon_yy", (np.abs, None))], [1, 0, 0, 0])
    system.create_term("sate", [("Ident", None)], [epsilont**3, 0, 0, 0])
    system.create_term("sate", [("epsilon_ext", (np.power, 3))], [1/8, 0, 0, 0])
    # Define gradients of c
    system.create_term("iqxc", [("c", None)], [1, 0, 1, 0])
    system.create_term("iqyc", [("c", None)], [1, 0, 0, 1])
    system.create_term("satc", [("Ident", None)], [ct*ct, 0, 0, 0])
    system.create_term("satc", [("iqxc", (np.power,2))], [1, 0, 0, 0])
    system.create_term("satc", [("iqyc", (np.power,2))], [1, 0, 0, 0])
    # Define Hxx
    system.create_term("Hxx", [("Qxx", None)], [a, 0, 0, 0])
    system.create_term("Hxx", [("S2", None), ("Qxx", None)], [-b, 0, 0, 0])
    system.create_term("Hxx", [("Qxx", None)], [-K, 1, 0, 0])
    system.create_term("Hxx", [("iqxc", (np.power, 2)), ("satc", (np.power, -1))], [gamma/2, 0, 0, 0])
    system.create_term("Hxx", [("iqyc", (np.power, 2)), ("satc", (np.power, -1))], [-gamma/2, 0, 0, 0])
    system.create_term("Hxx", [("epsilon_xx", None)], [chi/2, 0, 0, 0])
    system.create_term("Hxx", [("epsilon_yy", None)], [-chi/2, 0, 0, 0])
    # Define Hxy
    system.create_term("Hxy", [("Qxy", None)], [a, 0, 0, 0])
    system.create_term("Hxy", [("S2", None), ("Qxy", None)], [-b, 0, 0, 0])
    system.create_term("Hxy", [("Qxy", None)], [-K, 1, 0, 0])
    system.create_term("Hxy", [("iqxc", None), ("iqyc", None), ("satc", (np.power, -1))], [gamma, 0, 0, 0])
    system.create_term("Hxy", [("epsilon_xy", None)], [chi, 0, 0, 0])

    # Create terms for rho timestepping
    system.create_term("c", [("c", None)], [-1/tauc, 0, 0, 0])
    system.create_term("c", [("c", None)], [-D/tauc, 1, 0, 0])
    system.create_term("c", [("epsilon_ext", (np.power, 3)), ("sate", (np.power, -1))], [beta/(8*tauc), 0, 0, 0])

    # Create terms for Qxx timestepping
    system.create_term("Qxx", [("Hxx", None)], [1, 0, 0, 0])

    # Create terms for Qxy timestepping
    system.create_term("Qxy", [("Hxy", None)], [1, 0, 0, 0])
    ...
    exy      = system.get_field('epsilon_xy')
    ux      = system.get_field('ux')
    uy      = system.get_field('uy')

    # Initialise c
    c.set_real(0.01*np.random.rand(mx, my)+0.02)
    c.synchronize_momentum()

    #ux.set_real(0.1*np.random.rand(mx, my)+0.2)
    #ux.synchronize_momentum()

    #uy.set_real(0.1*np.random.rand(mx, my)+0.2)
    #uy.synchronize_momentum()
    # Initialise Qxx and Qxy
    #itheta = 2 * np.pi * np.random.rand(mx, my)
    #iS = np.tanh(np.random.rand(mx, my)+1)
    #Qxx.set_real(iS*(np.cos(itheta))/2)
    #Qxx.synchronize_momentum()
    #Qxy.set_real(iS*(np.sin(itheta))/2)
    ...
    system.get_field('sate').set_real(np.ones(shape=grid_size))
    system.get_field('sate').synchronize_momentum()
    ...
    for t in tqdm(range(n_steps)):
        ...
        if t % dn_dump == 0:
            ...
            np.savetxt(savedir+'/data/'+'ux.csv.'+ str(t//dn_dump), ux.get_real(), delimiter=',')
            np.savetxt(savedir+'/data/'+'uy.csv.'+ str(t//dn_dump), uy.get_real(), delimiter=',')
            np.savetxt(savedir+'/data/'+'exx.csv.'+ str(t//dn_dump), exx.get_real(), delimiter=',')
