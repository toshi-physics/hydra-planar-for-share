...
def main():
    ...
    # Define strains
    system.create_term("epsilon_xx", [("epsilon_xx", None)], [1, 0, 0, 0])
    system.create_term("epsilon_yy", [("epsilon_yy", None)], [1, 0, 0, 0])
    system.create_term("epsilon_xy", [("epsilon_xy", None)], [1, 0, 0, 0])
    # Define epsilon_iso
    system.create_term("epsilon_iso", [("epsilon_xx", None)], [1, 0, 0, 0])
    system.create_term("epsilon_iso", [("epsilon_yy", None)], [1, 0, 0, 0])
    system.create_term("epsilon_e", [("epsilon_iso", None)], [0.5/epsilont, 0, 0, 0])
    system.create_term("epsilon_e", [("epsilon_iso", (np.abs, None))], [0.5/epsilont, 0, 0, 0])
    ...
    system.create_term("c", [("c", None)], [-D, 1, 0, 0])
    system.create_term("c", [("epsilon_e", (np.tanh, None))], [beta, 0, 0, 0])
    ...
    #set_pair(Qxx, Qxy, grid_size, dr, nem_length=np.sqrt(K/a))
    ...
    inv_ematrix = invert_strain_matrix(k_grids, mu, lambd, B)
    ...
    for t in tqdm(range(n_steps)):
        ...
        update_strains(exx, eyy, exy, Qxx, Qxy, alpha, inv_ematrix, k_grids)
...
def set_pair(fieldxx, fieldxy, size, dr, nem_length):
    center1 = dr*size/2 + np.array([nem_length*30, 0])
    center2 = dr*size/2 - np.array([nem_length*30, 0])
    ...
    theta1 = np.arctan2((r[1]-center1[1]),(r[0]-center1[0]))/2
    theta2 = -np.arctan2((r[1]-center2[1]),(r[0]-center2[0]))/2
...
def invert_strain_matrix(k_grids, mu, lambd, B):
    qx = k_grids[0]
    qy = k_grids[1]
    e_matrix = np.array([[B+(lambd+2*mu)*qx**2, lambd*qx**2, 2*mu*qx*qy], [lambd*qy**2, B+(2*mu+lambd)*qy**2, 2*mu*qx*qy], [(lambd+mu)*qx*qy, (lambd+mu)*qx*qy, B+mu*(qx**2+qy**2)]]) #this has shape [3_row, 3_col, nx, ny]
    e_matrix = np.transpose(e_matrix, (2, 3, 0, 1)) #this has the shape [nx, ny, 3_row, 3_col]
    #e_matrix = np.array([[k+(lambd+2*mu)*qx**2, lambd*qy**2, (lambd+mu)*qx*qy], [lambd*qx**2, k+(2*mu+lambd)*qy**2, (lambd+mu)*qx*qy], [2*mu*qx*qy, 2*mu*qx*qy, k+mu*(qx**2+qy**2)]]).T
    inv_ematrix = np.linalg.inv(e_matrix) #invert 3x3 matrix for all qx,qy
    return inv_ematrix

def update_strains(fexx, feyy, fexy, fQxx, fQxy, alpha, inv_ematrix, k_grids):
    qx = k_grids[0]
    qy = k_grids[1]
    qQxx = fQxx.get_momentum()
    qQxy = fQxy.get_momentum()
    
    # shape 3,nx,ny
    b_vector = np.array([-alpha*qQxx*qx**2-alpha*qx*qy*qQxy, alpha*qQxx*qy**2-alpha*qx*qy*qQxy, -0.5*alpha*(qx**2+qy**2)*qQxy]) 
    b_vector = np.transpose(b_vector, (1,2,0)) #shape nx, ny, 3_col
    
    e_vector = np.matvec(inv_ematrix, b_vector)
    
    fexx.set_momentum(e_vector[:,:,0])
    fexx.synchronize_real()
    feyy.set_momentum(e_vector[:,:,1])
    feyy.synchronize_real()
    fexy.set_momentum(e_vector[:,:,2])
    fexy.synchronize_real()
