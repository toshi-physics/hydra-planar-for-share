...
def main():
    ...
    k         = parameters["k"]        # elastic coupling constant
    ...
    system.create_field('ux', k_list, k_grids, dynamic=True)
    system.create_field('uy', k_list, k_grids, dynamic=True)
    ...
    # ux timestepping
    system.create_term("ux", [("ux", None)], [-k/epsilon, 0, 1, 0])
    system.create_term("ux", [("sigma_xx", None)], [1/epsilon, 0, 1, 0])
    system.create_term("ux", [("sigma_xy", None)], [1/epsilon, 0, 0, 1])
    # uy timestepping
    system.create_term("uy", [("uy", None)], [-k/epsilon, 0, 1, 0])
    system.create_term("uy", [("sigma_yy", None)], [1/epsilon, 0, 0, 1])
    system.create_term("uy", [("sigma_xy", None)], [1/epsilon, 0, 1, 0])

    # Create terms for rho timestepping
    system.create_term("c", [("c", None)], [-1/tauc, 0, 0, 0])
    system.create_term("c", [("c", None)], [-D/tauc, 1, 0, 0])
    system.create_term("c", [("epsilon_e", (np.tanh, None))], [beta/tauc, 0, 0, 0])
