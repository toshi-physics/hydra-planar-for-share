    ...
    def update_system(self, dt):
        ...
        for field_name in self.fields:
            ...
            if self.fields[field_name].is_dynamic:
                absolute_error = self.aux_fields[field_name].get_real() - self.fields[field_name].get_real() 
                tolerance = self.atol + self.rtol*np.abs(self.fields[field_name].get_real())
                norm_error = np.abs(absolute_error)/tolerance
                # here we will be taking the maximum norm of the error
                dt_next = np.minimum(dt_next, dt * min(1.2, max(0.5, 0.8*np.power(1/np.max(norm_error), 1/self.RK_index))))
                # don't decrese the timestep by more than half or increase it by more than 1.2. 0.8 is safety factor.

        dt_next = np.maximum(dt_next, self.minstep)

        for field_name in self.aux_fields: #set aux fields' base values to that of the real fields' value for next step. This is imp.!
            self.aux_fields[field_name].set_real(self.fields[field_name].get_real())
            self.aux_fields[field_name].synchronize_momentum()
